...
(defconst bc-mode-version "1.0" "The current version of bc-mode")
...
;;;   --------------
;;
;; This package is known to work with GNU Emacs 19.28, through 19.32,
;; and XEmacs 19.14.  It may work with others, I haven't tried it.
;;
...
;;
;; Further, it would be nice to be able to support banner comments
;; with faux end delimiters when the comment syntax does not use an
;; end delimiter, e.g., in Lisp:
;;
;;    (setq bc-mode t)                ;;===================================;;
;;    (bc-select-keymap t)            ;;  This is a Banner comment with    ;;
;;    (bc-install-menubar)            ;;  faux end-delimiters              ;;
...
;; bc-G-rm-minus-stuff.
;;
;; Fix `do-auto-fill' so that it works properly when the line needs to
;; be wrapped onto more than one successive line.
;;
...
;;
;; Could add text properties to comments to record info about how they
;; were formatted.  For example, a property `bc-column' might tell in
;; what column the comment is supposed to start.  Functions like
;; `do-auto-fill' and `indent-sexp' could examine this.  Auto filling
;; could use the value of the `bc-column' property to realign the
;; comment, which probably got messed up when code text was typed.  It
;; could also use this value as a fill column for the code on that
;; line.
;;
;;
;;;_  * Could Do:
...
-----------------------------------
  ...
  BC-mode determines the following things by examining the comment:
    ...
    whitespace before the end delimiter.  In languages that have end
    delimiters, at least one space will separate the delimiter from
    the widest line of text.  Additional spaces are added as necessary
    to align the end delimiters to `bc-right-margin'.
...
(if bc-mode-basic-map
  ...
  (define-key bc-mode-basic-map "\C-xr\C-b" 'bc-submit-bug-report)
  (define-key bc-mode-basic-map "\C-xr\t" 'bc-set-tab-spacing)
  (define-key bc-mode-basic-map "\C-xr\C-f" 'bc-set-margin)
  (define-key bc-mode-basic-map "\C-xr\C-k" 'bc-toggle-keymap-level)
  (define-key bc-mode-basic-map "\C-xr\C-a" 'bc-toggle-auto-fill-advice)
  (define-key bc-mode-basic-map "\M-Q" 'bc-fill)
  (define-key bc-mode-basic-map [(meta return)] 'bc-fill)
  (define-key bc-mode-basic-map "\M-Z" 'bc-squeeze)
  (define-key bc-mode-basic-map "\M-T" 'bc-taller)
  (define-key bc-mode-basic-map "\M-S" 'bc-shorter)
  (define-key bc-mode-basic-map "\M-N" 'bc-narrower)
  (define-key bc-mode-basic-map "\M-W" 'bc-wider)
  (define-key bc-mode-basic-map "\M-U" 'bc-raise)
  (define-key bc-mode-basic-map "\M-R" 'bc-raise-flow)
  (define-key bc-mode-basic-map "\M-D" 'bc-lower)
  (define-key bc-mode-basic-map "\M-L" 'bc-lower-flow)
  (define-key bc-mode-basic-map "\M-K" 'bc-lower-flow)
  (define-key bc-mode-basic-map "\C-c\C-w" 'bc-kill-comment)
  (define-key bc-mode-basic-map "\C-c\M-w" 'bc-copy-comment-comment-as-kill)
  (define-key bc-mode-basic-map "\C-c\C-y" 'bc-yank)
  (define-key bc-mode-basic-map "\C-c\C-p" 'bc-yank-pop)
  (define-key bc-mode-basic-map "\C-c\C-o" 'bc-open-line)
  (define-key bc-mode-basic-map "\C-c\C-m" 'bc-newline)
  (define-key bc-mode-basic-map "\C-c\C-j" 'bc-indent-new-comment-line)
  (define-key bc-mode-basic-map "\C-c\C-k" 'bc-kill-line)
  (define-key bc-mode-basic-map "\M-B" 'bc-backward-comment)
  (define-key bc-mode-basic-map "\M-F" 'bc-forward-comment)
  (define-key bc-mode-basic-map "\M-E" 'bc-forward-comment)
  (define-key bc-mode-basic-map "\C-c\t" 'bc-indent-according-to-mode)
  (bc-mode-fsf-menu "Comments" bc-mode-basic-map))

; (or (assq 'bc-mode-use-basic-map minor-mode-map-alist)
;     (setq minor-mode-map-alist
; 	  (cons (cons 'bc-mode-use-basic-map bc-mode-basic-map)
...
(if bc-mode-extended-map
  ...
  (substitute-key-definition 'indent-new-comment-line 
			     ...
			     bc-mode-extended-map global-map)
  (define-key bc-mode-extended-map "\C-c\M-j" 'indent-new-comment-line)
  (define-key bc-mode-extended-map "\M-y" 'bc-yank-pop)
  (define-key bc-mode-extended-map "\t" 'bc-indent-according-to-mode)
  (define-key bc-mode-extended-map "\C-c\t" 'indent-according-to-mode)
  ;; Emacs 19 defines menus in the mode map. This call will return
  ;; t on Emacs 19, otherwise no-op and return nil.
  (bc-mode-fsf-menu "Comments" bc-mode-extended-map))

; (or (assq 'bc-mode-use-extended-map minor-mode-map-alist)
;     (setq minor-mode-map-alist
; 	  (cons (cons 'bc-mode-use-extended-map bc-mode-basic-map)
; 		minor-mode-map-alist)))
...
; (defvar bc-mode-use-basic-map nil
;   "This is the variable referenced in `minor-mode-map-alist' to
;   control activation of bc-mode-basic-map.  Only `bc-select-keymap'
;   should set this.")
; (make-variable-buffer-local 'bc-mode-use-basic-map)

; (defvar bc-mode-use-extended-map nil
;   "This is the variable referenced in `minor-mode-map-alist' to
;   control activation of bc-mode-extended-map.  Only `bc-select-keymap'
;   should set this.")
; (make-variable-buffer-local 'bc-mode-use-extended-map)
...
If a comment starts in the first column, it is treated as code."
  (let (code-width)
    (bc-parse-line nil nil nil nil nil 'code-width)
    code-width))
  ...
  (cond
   ...
   (t
    ...
    (let* ((current-point (point))
	   ...
	   (comment-p (bc-parse-line nil 'delim-b 'body-b nil 'delim-e))
...
;;;	(current-point (point))
	...
	(save-line (bc-count-lines-1 (point-min) (point)))
...
;;;	 code-end body-e
	 ...
	 (save-column (current-column))
	 (comment-p (bc-parse-line nil 'delim-b 'body-b 'body-e 
				   'delim-e))
	 new-range first-line last-line
	 )
		...
		(save-excursion			;==============================
		  (setq 
		   ...
		   max-code-width (+ current-start-column 
				     bc-code-width-threshold) 
		   new-delims (list current-start-column
				    (if trim-whitespace 
					current-start-delim-sans-white
				      current-start-delim)
				    bc-comment-end))		; Fix up the
		  ...
		  (while (< 0 comment-point-line)
		    ...
		    (forward-line 1))
		  (setq save-line (bc-count-lines-1 1 (point))
...
;;;	 (save-line (bc-count-lines-1 1 (point)))
	 (save-column (current-column))
	 (comment-p (bc-parse-line 'code-end 'delim-b 'body-b nil 'delim-e))
	 new-range
	 )
...
For more information, type `\\[describe-variable] bc-mode-doc'."
  ...
  (let* (code-end start-delim no-code temp
	 ...
	 (mend (make-marker))
	 (comment-p
	  (bc-parse-line 'code-end 'start-delim nil nil nil nil 'no-code))
...
;	    (delete-horizontal-space)
	    ...
	    (bc-parse-line nil 'start-delim)		; changed so flush
...
For more information, type `\\[describe-variable] bc-mode-doc'."
  ...
  (let ((point (point))
	...
	(beginning-of-line)
      ...
      (if (bc-parse-line nil 'delim-b 'comment-b)
	  (cond
	   ((< point delim-b)
	    (beginning-of-line))
	   (t
...
;;;	comment-b delim-e
	...
	(end-of-line)
      (bc-flush-parse-cache)
      (if (bc-parse-line 'code-e 'delim-b nil 'comment-e)
	  (cond
	   ((< point delim-b)
	    (goto-char code-e))
	   (t
...
For more information, type `\\[describe-variable] bc-mode-doc'."
  ...
  (let* ((point (point))
	 body-b body-e
	 (comment-p (bc-parse-line nil nil 'body-b 'body-e))
	...
	(progn
	  ...
	  (if move-on nil
	    (bc-parse-line nil nil 'body-b 'body-e)
    ...
    (while (not (or (and (= direction 1) (eobp))
      ...
      (while (not (or (eobp) (bobp) (bc-parse-line)))
	(forward-line direction))
      (if (or (eobp) (bobp)) nil
	(bc-msetq (first-line last-line) (bc-find-comment))
;;	  (setq range (bc-find-comment)
...
;;;;========================================================================
;;;_    | bc-select-keymap
(defun bc-select-keymap (on-off)
  "Select a minor mode map according to the value of `bc-keymap-level'.
If ON-OFF is nil, turn off the minor mode maps.  If non-nil then set
either `bc-mode-use-basic-map' (if `bc-keymap-level' is nil), or 
`bc-mode-use-extended-map' (if `bc-keymap-level' is non-nil)."
  
;   (if on-off
;       (if bc-keymap-level
; 	  (setq bc-mode-use-basic-map nil
; 		bc-mode-use-extended-map t)
; 	(setq bc-mode-use-basic-map t
; 	      bc-mode-use-extended-map nil))
;     (setq bc-mode-use-basic-map nil
; 	  bc-mode-use-extended-map nil)))
  (setq bc-mode-map
  ...
  (if on-off
      (setq
       ...
       minor-mode-map-alist
       (let ((map (if bc-keymap-level bc-mode-extended-map bc-mode-basic-map)))
	 (cons (cons 'bc-mode map) minor-mode-map-alist)))))
...
;;;    ok code-end narrowed-area range no-code-after new-range initial-delta
;;;       first-line-char last-line-char
;;;       body-range
       )
...
;;; old-comment-beg-line old-comment-end-line	; we are about to delete some
	...
	(before-change-fun before-change-function)
	(after-change-funs after-change-functions)
	(after-change-fun after-change-function)
;;;	(code-buffer (current-buffer))
;;;	(scratch-buffer (bc-get-buffer-create))
...
;;;	com-delim-end comment-start-col comment-end-col
	...
	(bc-parse-line nil nil 'com-body-start 'com-body-end
		       'com-delim-end)
...
;;;	 eol 
  ...
  (save-excursion
    ...
    (while (> num-lines 0)
      ...
      (bc-internal-end-of-line)
;;;      (setq eol (point))
      (bc-parse-line 'code-end-pos nil nil nil 'delim-e)
...
;	   (bc-fill-or-tab-macro)
    ...
    (cond
     ...
     ((eq need-blank-line 'line-too-wide)
      (goto-char orig-top)
      (forward-line -1)
      (if (bc-parse-line) (setq need-blank-line 'comment-above)))
     )
	...
	(setq where -1)
	(goto-char orig-top)
	(bc-parse-line nil nil nil nil nil nil 'no-code-this-line)
	 ...
	 (setq
	  ...
	  (catch 'no-more-down		;; there is enough space in that
		...
		(if (eobp) (throw 'no-more-down 'bottom-of-buffer)))
	      ...
	      (forward-line 1)
	      (bc-parse-line 'code-end-pos nil nil nil nil 
...
;;;	 range-top top-is-comment bottom-is-comment 
    ...
    (if (not (bc-parse-line nil nil nil nil nil nil	; Is there a comment on
...
;;;	 bottom-is-comment code-end-pos com-delim-start-pos 
    ...
    (save-excursion
      (while (and
	      ...
	      (not (setq top-is-comment (bc-parse-line)))))
      (goto-char range-top)
...
;;;	bol bottom-is-comment
    ...
    (if (not (bc-parse-line))		     ; Is there a comment on this line?
	(throw 'failure "No block comment found."))
...
(defmacro bc-analyze-line ()
  (`
   (save-excursion 
     (bc-parse-line nil 'start-delim-b 'body-b 'body-e 'end-delim-e)
     (goto-char body-b)
     (skip-chars-backward " \t")
...
Also determines whether the comment has a banner.
...
;;;	bannerp nonwhite
	...
	(bc-parse-line nil nil 'body-b 'body-e))
      ...
      (if (not (bc-parse-line nil 'start-delim-b 'body-b))
	  (progn
	    (setq new-end end
	...
	(setq						; effective start and
	 ...
	 bc-effective-end (if (equal bc-comment-end "") ""
	...
	(while (and (< new-start (point)) (= body-b body-e))
	  (forward-line -1)
	  (bc-parse-line nil nil 'body-b 'body-e))	; Skip blank lines at
	(setq new-end (max (point) new-start))		; bottom.

	);; end (if (not (bc-parse-line)
	...
	(while (< new-end (point))
	  ...
	  (forward-line -1)))
							; Figure out comment
      (while (and (<= new-start (point))		; width.
		  (not (bobp)))
	(bc-parse-line nil 'start-delim-b 'body-b 'body-e 'end-delim-e)
...
;;;
      (while (and save-delims (<= start (point)))	; Save delims in the
	(bc-parse-line nil 'start-delim-b 'body-b	; top banner border.
		       'body-e 'end-delim-e)
	(bc-save-banners)
	(forward-line -1))
...
;;;;======================================================================
;;;_    | bc-parse-line
(defun bc-parse-line (&optional bpl-code-end bpl-delimiter-start 
  ...
  (save-excursion 
	...
	(setq bpli-commentp
		...
		(re-search-forward bc-comment-regexp bpli-eol 'move))
	      ...
	      bpli-body-end (and bpli-commentp
				 (progn			       ; Might be extra
				   (goto-char (match-end 1))   ; end delim
				   (skip-chars-backward	       ; chars that got
				    (char-to-string	       ; matched by the
				     (following-char)))	       ; .*
				   (skip-chars-backward " \t")
				   (max (point) (match-beginning 1))))
...
location of the comment."
  ...
  (let ((vert (car save-point))
	...
	(cond
	 ((eq horiz 'in-start-delimiter)	; Point in start delimiter.
	  (if (bc-forward-nonwhite body-first (car pos)) nil
	    (bc-parse-line nil 'com-delim-start-pos 'com-body-start-pos)
	 ...
	 (t					; Point in end delimiter.
	  ...
	  (bc-parse-line nil nil nil nil 'com-delim-end-pos)
	...
	(bc-parse-line nil 'com-delim-start-pos nil 'com-body-end-pos
		       'com-delim-end-pos)
	(cond
...
;;;       code-end-pos 
    ...
    (save-excursion
      (goto-char first-line)
      (while (not done)
	(bc-parse-line nil nil 'com-body-start-pos 'com-body-end-pos)
...
;;;       code-end-pos com-delim-start-pos com-delim-end-pos
    ...
    (goto-char first-line)
    (while (not done)
      (if (not (bc-parse-line nil nil 'com-body-start-pos 'com-body-end-pos))
	  (progn
	    (setq done t
		  failure t)
	    (goto-char last))
