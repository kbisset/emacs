...
;;  Keywords:    News, RSS
;;  Time-stamp:  "6. November 2003, 19:31:18 (ulf)"
;;  CVS-Version: $Id: newsticker.el,v 1.24 2003/11/06 18:29:45 u11 Exp $

(defconst newsticker-version "1.3" "Version number of newsticker.el.")
...
;;; History:
;;
;;  1.3  Compare title AND desc to check whether item is old, except for
;;         feed desc
...
;;(defvar newsticker--growing-message-actual-size 0
;;  "Actual size of currently displayed item title.")
;;(defvar newsticker--growing-message-total-size 0
;;  "Total size of currently displayed item title.")
...
(defvar newsticker--font-lock-keywords
  '(("^[*]   \\(.*\\) ([^)]+)"     1 'newsticker-feed-face)
    ...
    ("^!!  \\(.*\\) ([^)]+)"       1 'newsticker-important-item-face)
    ("^    \\(.*\\)"               1 'newsticker-default-face))
  ;; The more restrictive version which did not allow anything after the
  ;; closing bracket did not work with XEmacs when items were folded.
  "Font lock keywords for the newsticker buffer.")
...
(define-derived-mode newsticker-mode outline-mode
  ...
  ;; we do not want the default outline-regexp
...
;;(define-key newsticker-mode-map "!" 
;;  'newsticker-toggle-item-importance-at-point)
...
;;(define-key newsticker-menu [newsticker-toggle-item-importance-at-point]
;;  '("Toggle importance/coolness/immortality of the item at point" .
;;    newsticker-toggle-item-importance-at-point))
...
;; (defun newsticker-toggle-item-importance-at-point ()
;;   "Toggle importance/coolness/immortality of the item at point"
;;   (interactive)
;;   (let ((feed (get-text-property (point) 'feed))
;; 	title desc)
...
Argument EVENT tells what has happened to the process."
  (let* ((p-status (process-status process))
    ...
    (catch 'oops
	  ...
	  (when coding-system
	    (setq title
		  (decode-coding-string title coding-system))
	    (if desc
		(setq desc
		      (decode-coding-string desc
					    coding-system)))
	    (setq link
		  (decode-coding-string link coding-system)))
...
;; (defun newsticker--display-grow ()
;;   "Called from the display timer.
;; This function displays the next ticker item in the echo area, unless
;; there is another message displayed or the minibuffer is active."
;;   (let ((message-log-max nil));; prevents message text from being logged
;;     (when (newsticker--echo-area-clean-p)
;;       (when (> newsticker--growing-message-actual-size
;; 	       newsticker--growing-message-total-size)
;; 	(setq newsticker--item-position (1+ newsticker--item-position))
;; 	(when (>= newsticker--item-position (length newsticker--item-list))
;; 	  (setq newsticker--item-position 0))
;; 	(setq newsticker--growing-message-actual-size 0)
;; 	(setq newsticker--growing-message-total-size 
;; 	      (+ 5 (length (nth newsticker--item-position
;; 				newsticker--item-list)))))
;;       (setq newsticker--growing-message-actual-size
;; 	    (1+ newsticker--growing-message-actual-size))
;;       (setq newsticker--prev-message
;; 	    (substring (nth newsticker--item-position newsticker--item-list)
;; 		       0 newsticker--growing-message-actual-size))
;;       (message newsticker--prev-message))))
...
(defun newsticker--cache-contains (data feed title desc mark)
  "Check DATA whether the FEED contains an item with TITLE and DESC.
If MARK equals 'feed the DESCription does not matter.
        ...
        (mapcar (lambda (this-feed)
                  (when (eq (car this-feed) feed)
                    (mapcar (lambda (anitem)
                              (when (and (string= (nth 0 anitem) title)
					 (or (eq mark 'feed)
					     (string= (nth 1 anitem) desc)))
                                (throw 'found anitem)))
                            (cdr this-feed))))
                data)
        nil)
