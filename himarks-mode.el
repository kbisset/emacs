...
;;
;;     (define-key global-map (kbd "C-x SPC")
;;       'himarks-set-mark-no-activate)
;;
...
;;; TODO:
;;
;; * If you push mark at same position where is current marker,
;;   himarks can't track this.
;;     - Done, now `set-mark' and `pop-mark' adviced. --ZajcevEvgeny
;; 
...
(defun himarks-show-markers (buffer)
  ...
  (with-current-buffer buffer
    (himarks-highlight-char (marker-position (mark-marker t))
'himarks-mark-face)
...
(defadvice set-mark (after unused activate)
  "Tracker for `himarks-mode'."
  (when himarks-mode
    (himarks-hide-markers (current-buffer))
    (himarks-show-markers (current-buffer))))

(defadvice pop-mark (after unused activate)
  "Tracker for `himarks-mode'."
  (when himarks-mode
    (himarks-hide-markers (current-buffer))
    (himarks-show-markers (current-buffer))))
