...
;; RCS $Id: word-help.el,v 1.30 1997/05/28 12:32:47 jensthi Exp $
...
Currently supported  kyes are (with the \"variables\" they refer to - do
...
Case-insensitive is IGNORE-CASE is non-nil."
  (let* (cmd sym)
    (and
...
(defun word-help-extract-index (index-ob file index ignore-case index-map)
  (let (topic keyword)
    (message "Processing \"%s\" in %s..." index file)
    (save-window-excursion
      (Info-goto-node (concat "(" file ")" index))
      (end-of-buffer)
      (while (re-search-backward "\\* \\([^\n:]+\\):" nil t)
	(setq topic (buffer-substring (match-beginning 1) (match-end 1)))
	(setq keyword (word-help-map-index-entries topic index-map))
	(word-help-keyword-insert index-ob keyword file index topic ignore-case)))))
	    ...
	    (cond
	     ...
	     (t
	      (let ((regexp (car cmpl))
		    ...
		    (cursm (cond ((nth 2 cmpl)) (0)))
		    (app (cond ((nth 3 cmpl)) ("")))
		    (re-list (cond ((nth 4 cmpl)) ('(("."))))))
