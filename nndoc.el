...
(defvar nndoc-type-alist
  `((mmdf
    ...
    (lanl-sc-announce
     ...
     (body-end   . "-------------------------------------------------")
;;     (file-end   . "^Title: Recent Seminal")
     (generate-head-function . nndoc-generate-lanl-sc-head)
     (article-transform-function . nndoc-transform-lanl-sc-announce)
     (subtype preprints guess))
    (rfc822-forward
     (article-begin . "^\n")
...
(defun nndoc-lanl-sc-announce-type-p ()
  (when (let ((case-fold-search nil))
	  (re-search-forward "^[0-9]+ citations matching your alert:$" nil t))
    t))

(defun nndoc-transform-lanl-sc-announce (article)
  (goto-char (point-max))
  (when (re-search-backward "^\\\\\\\\ +(\\([^ ]*\\) , *\\([^ ]*\\))" nil t)
    (replace-match "\n\nGet it at \\1 (\\2)" t nil))
  ;;  (when (re-search-backward "^\\\\\\\\$" nil t)
...
(defun nndoc-generate-lanl-sc-head (article)
  (let ((entry (cdr (assq article nndoc-dissection-alist)))
 	(e-mail "no address given")
 	subject from)
    (save-excursion
