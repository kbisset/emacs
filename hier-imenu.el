...
;; Copyright (C) 1996 Y.Dirson

;; Author: Y.Dirson <en581@cleveland.freenet.edu> <dirson@univ-mlv.fr>
;; Snail-Mail: 18 rue du Mj Allard, 49100 Angers, FRANCE
;; Keywords: tools hypermedia tex
;; Created: July 1996
;; URL: http://monge.univ-mlv.fr/~dirson/files/hier-imenu.el
;; Required-URL: http://monge.univ-mlv.fr/~dirson/files/imenu.el
;; RCS: $Id: hier-imenu.el,v 1.13 1996/11/10 18:37:57 dwitch Exp $
...
;;
;; WARNING: support for LaTeX-mode will not work unless you upgrade
;; `imenu.el' !! (see Required-URL on top of this file)
;;
...
;; this last functionnality you need a newer version of `imenu.el'
;; than the one supplied with emacs-19.31. See `Required-URL' at the
;; top of this file.
...
;;
;; * should detect and handle `case-fold-search' ? With which
;; semantics ?
...
;;
;; * fix order of parameters for functions
;;
;; * add a `global-imenu-add-to-menu-bar' function (see font-lock.el).
	    ...
	    (mapcar (lambda (el)
			...
			(if (or (null hier-imenu-max-numbering-depth)
			    ...
			    (while (>= loop-index min-level)
			      (setq secnum 
				    (if secnum
					(concat (princ (aref sec-num loop-index))
						"." (princ secnum))
				      (aref sec-num loop-index)))
			      (setq loop-index (1- loop-index))))
...
(defmacro ydi-latex-hook ()
  (function (lambda ()
	      (setq hier-imenu-levels '(("\\part" . 0)		("\\part*" . 0)
				       ...
				       ("\\paragraph" . 5)
				       ("\\subparagraph" . 6))
		    hier-imenu-header-name-function 'ydi-latex-header-name
		    hier-imenu-anchor-end-of-header t
		    hier-imenu-style 'numbered
		    imenu-create-index-function 'imenu-default-create-index-function
		    imenu-generic-expression '(("Labels" "\\\\label{\\([^}]+\\)}" 1)
					       (imenu-create-hierarchical-index . nil))
		    ))))


;; standard LaTeX mode and AUC-TeX  
(add-hook 'tex-mode-hook (ydi-latex-hook))
(add-hook 'TeX-mode-hook (ydi-latex-hook))

;; cleanup
(fmakunbound 'ydi-latex-hook)



;;;; HTML-mode:
(defun ydi-html-header-name ()
...
Assume the point is just before the opening angle-braquet."
  (buffer-substring-no-properties
   (progn
     ...
     (point))
   (progn
     (re-search-forward "</[Hh]")
     (match-beginning 0))
   ))

(defmacro ydi-html-hook ()
  (function (lambda ()
	      (setq hier-imenu-levels
		    '(("<H1" . 0)("<h1" . 0)
		      ...
		      ("<H5" . 4)("<h5" . 4)
		      ("<H6" . 5)("<h6" . 5))
		    hier-imenu-header-name-function 'ydi-html-header-name
		    hier-imenu-anchor-end-of-header t
		    imenu-create-index-function 'imenu-create-hierarchical-index))))

(add-hook 'html-mode-hook (ydi-html-hook))
(add-hook 'html-helper-hook (ydi-html-hook))

(fmakunbound 'ydi-html-hook)
